...
insert into ts_domains(ts_name, ts_ip_address, ts_port, ts_db_versions, ts_tran_importance_id, ts_user_importance_id, version_info, ts_dynamic_group_reassign) values ('Local Domain', 0, 80, '5.0.0.0', 5, 5, 1, false);
...
Insert into ts_apps(ts_id, ts_domain_id, ts_authtype_id, ts_apptype_id, ts_name, ts_description, ts_charset, ts_session_timeout, ts_user_processing_type, ts_case_sensitivity_url_host, ts_case_sensitivity_url_path, ts_case_sensitivity_query_name, ts_case_sensitivity_query_value, ts_case_sensitivity_cookie_name, ts_case_sensitivity_cookie_value, ts_case_sensitivity_post_name, ts_case_sensitivity_post_value, ts_case_sensitivity_login_name, ts_soft_delete, version_info) values(1, 1, 1, 2, 'Default Application', 'Default Application', 'ISO-8859-1', 60, 0, true, true, true, true, true, true, true, true, true, false, 1);
...
Insert into ts_impact_settings(ts_domain_id, ts_name, ts_weight, ts_color, ts_bitmap, ts_action_cmd, ts_action_enabled, ts_soft_delete, version_info) values(1, 'Moderate', 1000, 256*256*255+256*198+0, 'i_moderate.gif', null, false, false, 1);
Insert into ts_impact_settings(ts_domain_id, ts_name, ts_weight, ts_color, ts_bitmap, ts_action_cmd, ts_action_enabled, ts_soft_delete, version_info) values(1, 'Severe', 2500, 256*256*230+256*120+0, 'i_severe.gif', null, false, false, 1);
Insert into ts_impact_settings(ts_domain_id, ts_name, ts_weight, ts_color, ts_bitmap, ts_action_cmd, ts_action_enabled, ts_soft_delete, version_info) values(1, 'Critical', 5000, 256*256*180+256*40+40, 'i_critical.gif', null, false, false, 1);
...
-- Set the sequence value to restart
ALTER SEQUENCE ts_defect_meta_keys_ts_id_seq RESTART WITH 600000000000000000;
...
-- Set the sequence value to start after the last ID value
SELECT setval('ts_operators_ts_id_seq', (select max(ts_id) from ts_operators));
...
-- Set the sequence value to start after the last ID value
SELECT setval('ts_attributes_ts_id_seq', (select max(ts_id) from ts_attributes));
