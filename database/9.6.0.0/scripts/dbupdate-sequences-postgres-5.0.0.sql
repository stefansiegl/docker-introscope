...
-- Reset the sequence ts_apps_ts_id_seq
SELECT setval('ts_apps_ts_id_seq', 
				(SELECT   
			      CASE 
...
-- Reset the sequence ts_defect_meta_keys_ts_id_seq
SELECT setval('ts_defect_meta_keys_ts_id_seq', 
				(SELECT   
			      CASE 
...
SELECT setval('ts_hourly_aggregation_log_ts_id_seq', (select max(ts_id) from ts_hourly_aggregation_log));
