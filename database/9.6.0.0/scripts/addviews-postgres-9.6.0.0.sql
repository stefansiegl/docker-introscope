...
create or replace view TSGrpSlaPerfTimeSlaVw AS
 ...
 (SELECT g.ts_transet_group_id AS TranSetGroupId, i.ts_tran_tran_time_sla AS TranTimeSla
  ...
  WHERE f.ts_incarnation_id = g.ts_transet_incarnation_id AND f.ts_trandef_group_id = h.ts_id AND f.ts_tran_time_sla_inherited = true AND h.ts_tran_time_sla_inherited = true AND h.ts_domain_id = i.ts_id AND f.ts_soft_delete = false);
